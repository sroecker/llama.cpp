...
static size_t ggml_cuda_nvfp4_repack_cache_budget_from_env();
static size_t ggml_cuda_nvfp4_repack_cache_max_tensor_size_from_env();
...
struct ggml_backend_cuda_buffer_context {
    ...
    size_t nvfp4_repack_cache_max_tensor_size = SIZE_MAX;
    std::vector<ggml_cuda_nvfp4_repack_cache *> nvfp4_repack_caches;

    ggml_backend_cuda_buffer_context(int device, void * dev_ptr) :
        device(device), dev_ptr(dev_ptr),
        name(GGML_CUDA_NAME + std::to_string(device)) {
        nvfp4_repack_cache_budget = ggml_cuda_nvfp4_repack_cache_budget_from_env();
        nvfp4_repack_cache_max_tensor_size = ggml_cuda_nvfp4_repack_cache_max_tensor_size_from_env();
        if (nvfp4_repack_cache_budget > 0 && ggml_cuda_nvfp4_repack_debug_enabled()) {
            GGML_LOG_INFO("CUDA NVFP4 repack cache: budget %.2f MiB for %s buffer, max tensor %.2f MiB\n",
                nvfp4_repack_cache_budget / 1024.0 / 1024.0, name.c_str(),
...
static size_t ggml_cuda_nvfp4_repack_cache_budget_from_env() {
    ...
    return end != env ? size_t(mb) * 1024 * 1024 : 0;
}

static size_t ggml_cuda_nvfp4_repack_cache_max_tensor_size_from_env() {
    const char * env = getenv("GGML_CUDA_NVFP4_REPACK_CACHE_MAX_TENSOR_MB");
    if (env == nullptr || *env == '\0') {
        return SIZE_MAX;
    }
    ...
    char * end = nullptr;
    const unsigned long long mb = std::strtoull(env, &end, 10);
    return end != env ? size_t(mb) * 1024 * 1024 : SIZE_MAX;
}

static size_t ggml_cuda_nvfp4_repack_cache_nbytes_from_nblocks(const size_t nblocks) {
    constexpr int blocks_per_group = MMQ_ITER_K_FP4 / QK_NVFP4;
    constexpr int ints_per_group = MMQ_ITER_K_FP4 / 8 + blocks_per_group;

    if (nblocks % blocks_per_group != 0) {
        return 0;
    }
...
static size_t ggml_cuda_nvfp4_repack_cache_nbytes(const ggml_tensor * tensor) {
    ...
    return ggml_cuda_nvfp4_repack_cache_nbytes_from_nblocks(nblocks);
}
...
static enum ggml_status ggml_backend_cuda_split_buffer_init_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor) {
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        if (nvfp4_repack_size > 0 && ctx->nvfp4_repack_cache_budget > ctx->nvfp4_repack_cache_used &&
                ...
                nvfp4_repack_size <= ctx->nvfp4_repack_cache_budget - ctx->nvfp4_repack_cache_used) {
            void * cache_data = nullptr;
            cudaError_t err = ggml_cuda_device_malloc(&cache_data, nvfp4_repack_size, id);
            if (err == cudaSuccess) {
                ggml_cuda_nvfp4_repack_cache * cache = new ggml_cuda_nvfp4_repack_cache{};
                cache->data = cache_data;
                cache->size = nvfp4_repack_size;
                ...
                if (ggml_cuda_nvfp4_repack_debug_enabled()) {
                    GGML_LOG_INFO("CUDA NVFP4 repack cache: reserved %.2f MiB for split %s on device %d (used %.2f / %.2f MiB)\n",
                        nvfp4_repack_size / 1024.0 / 1024.0, tensor->name, id,
                        ctx->nvfp4_repack_cache_used / 1024.0 / 1024.0,
                        ctx->nvfp4_repack_cache_budget / 1024.0 / 1024.0);
                }
            } else {
                (void) cudaGetLastError();
                if (ggml_cuda_nvfp4_repack_debug_enabled()) {
                    GGML_LOG_WARN("CUDA NVFP4 repack cache: allocation of %.2f MiB failed for split %s on device %d: %s\n",
                        nvfp4_repack_size / 1024.0 / 1024.0, tensor->name, id, cudaGetErrorString(err));
                }
            }
        }
...
static void ggml_backend_cuda_split_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
        ...
        ggml_cuda_nvfp4_repack_cache * cache = extra->nvfp4_repack_cache[id];
        if (cache != nullptr) {
...
static void ggml_cuda_op_mul_mat_cublas(
    ...
    GGML_UNUSED_VARS(dst, src0_repacked_i, src1_ddq_i, src1_padded_row_size);
}
        ...
        if (src0_is_contiguous) {
            ...
            if (split && ggml_cuda_nvfp4_repack_split_cache_enabled() &&
                ...
                if (cache != nullptr && cache->ready) {
                    ...
                    dev[id].src0_repacked = true;

                    if (ggml_cuda_nvfp4_repack_debug_enabled()) {
                        static bool logged = false;
                        if (!logged) {
                            GGML_LOG_INFO("CUDA NVFP4 repack cache: using split-buffer MMQ sidecar weights\n");
                            logged = true;
                        }
                    }
                }
    ...
    for (int64_t src1_col_0 = 0; src1_col_0 < ne11; src1_col_0 += src1_col_stride) {
        ...
        for (int id = 0; id < ggml_backend_cuda_get_device_count(); ++id) {
            ...
            for (int64_t i0 = 0; i0 < ne13*ne12; ++i0) {
                ...
                // do the computation
                op(ctx, src0, src1, dst, src0_dd_i, dev[id].src0_repacked, src1_ddf_i, src1_ddq_i, dst_dd_i,
...
static const ggml_tensor * ggml_cuda_nvfp4_input_scale(const ggml_tensor * mm) {
    ...
    const ggml_tensor * input_scale = mm->src[3];
    if (!src0 || src0->type != GGML_TYPE_NVFP4 || input_scale->type != GGML_TYPE_F32) {
        return nullptr;
    }

    if (mm->op == GGML_OP_MUL_MAT) {
        return ggml_nelements(input_scale) == 1 ? input_scale : nullptr;
    }

    if (mm->op == GGML_OP_MUL_MAT_ID) {
        return mm->src[2] && input_scale->ne[0] == src0->ne[2] ? input_scale : nullptr;
    }
...
static bool ggml_cuda_should_fuse_mul_mat_q_glu(const ggml_tensor * tensor, const ggml_cuda_mm_fusion_args_host & fusion) {
    ...
    if (fusion.x_bias || fusion.gate_bias || !fusion.gate) {
        return false;
    }
    ...
    if (src0->type != GGML_TYPE_NVFP4 || fusion.gate->type != GGML_TYPE_NVFP4 ||
        src1->type != GGML_TYPE_F32 || dst->type != GGML_TYPE_F32) {
        return false;
    }

    if ((fusion.x_scale    && (fusion.x_scale->type    != GGML_TYPE_F32 || fusion.x_scale->ne[0]    != src0->ne[2])) ||
        (fusion.gate_scale && (fusion.gate_scale->type != GGML_TYPE_F32 || fusion.gate_scale->ne[0] != src0->ne[2]))) {
        return false;
    }
    ...
    const bool split = ggml_backend_buft_is_cuda_split(src0->buffer->buft) ||
                       ggml_backend_buft_is_cuda_split(src1->buffer->buft) ||
                       ggml_backend_buft_is_cuda_split(fusion.gate->buffer->buft) ||
                       (fusion.x_scale    && ggml_backend_buft_is_cuda_split(fusion.x_scale->buffer->buft)) ||
                       (fusion.gate_scale && ggml_backend_buft_is_cuda_split(fusion.gate_scale->buffer->buft));
    if (split) {
        return false;
    }
...
static void ggml_cuda_mul_mat(ggml_backend_cuda_context & ctx, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    const ggml_tensor * input_scale = ggml_cuda_nvfp4_input_scale(dst);
    ...
    if (!split && use_mul_mat_vec_f) {
        ...
        ggml_cuda_mul_mat_q(ctx, src0, src1, nullptr, dst, nullptr, input_scale);
...
static void ggml_cuda_mul_mat_id(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (src1->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
        ...
        if (ggml_cuda_should_use_mmq(src0->type, cc, ne12, /*n_experts=*/ne02)) {
            ggml_cuda_mul_mat_q(ctx, src0, src1, ids, dst, nullptr, ggml_cuda_nvfp4_input_scale(dst));
            return;
        }
...
struct ggml_cuda_scaled_mul_mat_id {
...
static bool ggml_cuda_extract_scaled_mul_mat_id(ggml_tensor * mul, ggml_cuda_scaled_mul_mat_id & result) {
    ...
    if (mul->src[0] && mul->src[0]->op == GGML_OP_MUL_MAT_ID && mul->src[1] && mul->src[1]->op == GGML_OP_GET_ROWS) {
        mm       = mul->src[0];
        get_rows = mul->src[1];
    } else if (mul->src[1] && mul->src[1]->op == GGML_OP_MUL_MAT_ID && mul->src[0] && mul->src[0]->op == GGML_OP_GET_ROWS) {
        mm       = mul->src[1];
        get_rows = mul->src[0];
    } else {
        return false;
    ...
    if (!mm->src[0] || !mm->src[1] || !mm->src[2] || get_rows->src[1] != mm->src[2]) {
        return false;
    }

    ggml_tensor * repeat = get_rows->src[0];
    if (!repeat || repeat->op != GGML_OP_REPEAT || !repeat->src[0] || repeat->src[0]->op != GGML_OP_RESHAPE) {
        return false;
    }

    ggml_tensor * scale = repeat->src[0]->src[0];
    if (!scale || scale->type != GGML_TYPE_F32 || scale->ne[0] != mm->src[0]->ne[2]) {
        return false;
    }
...
static bool ggml_cuda_should_fuse_scaled_mul_mat_q(
    ...
    const ggml_tensor * input_scale = ggml_cuda_nvfp4_input_scale(mm);
    if (mm->src[3] && !input_scale) {
        return false;
    }
    ...
    const bool split = ggml_backend_buft_is_cuda_split(src0->buffer->buft) ||
                       ...
                       (input_scale && ggml_backend_buft_is_cuda_split(input_scale->buffer->buft));
    if (split) {
        return false;
    }
...
static bool ggml_cuda_can_fuse_scaled_mul_mat_id_glu(const ggml_cgraph * cgraph, const int i) {
    ...
    int out_nodes[] = { i + 10 };
    return ops_match && uses_ok && ggml_cuda_check_fusion_memory_ranges(cgraph, i, 11, out_nodes, 1);
}
...
static int ggml_cuda_try_fuse(ggml_backend_cuda_context * cuda_ctx, ggml_cgraph * cgraph, int i) {
    ...
    for (ggml_op op : { GGML_OP_MUL_MAT, GGML_OP_MUL_MAT_ID }) {
        ...
        if (ggml_cuda_can_fuse(cgraph, i, { op, bias_op, op, bias_op, GGML_OP_GLU }, {})) {
        ...
        } else if (op == GGML_OP_MUL_MAT_ID && ggml_cuda_can_fuse_scaled_mul_mat_id_glu(cgraph, i)) {
            ggml_tensor * glu = cgraph->nodes[i + 10];
            GGML_ASSERT(glu->op == GGML_OP_GLU);

            ggml_cuda_scaled_mul_mat_id gate_info;
            ggml_cuda_scaled_mul_mat_id up_info;
            if (!ggml_cuda_extract_scaled_mul_mat_id(glu->src[0], gate_info) ||
                    !ggml_cuda_extract_scaled_mul_mat_id(glu->src[1], up_info)) {
                continue;
            }

            if (gate_info.mm->src[1] != up_info.mm->src[1] || gate_info.mm->src[2] != up_info.mm->src[2]) {
                continue;
            }

            const ggml_tensor * src0 = up_info.mm->src[0];
            const ggml_tensor * src1 = up_info.mm->src[1];
            const ggml_tensor * ids  = up_info.mm->src[2];

            const bool should_mmvq = ggml_cuda_should_fuse_mul_mat_vec_q(up_info.mm);
            if (should_mmvq) {
                ggml_cuda_mm_fusion_args_host fusion_data{};
                fusion_data.gate       = gate_info.mm->src[0];
                fusion_data.x_scale    = up_info.scale;
                fusion_data.gate_scale = gate_info.scale;
                fusion_data.glu_op     = ggml_get_glu_op(glu);

                if (!up_info.mm->src[3] && !gate_info.mm->src[3] && ggml_cuda_should_fuse_mul_mat_q_glu(up_info.mm, fusion_data)) {
                    ggml_cuda_mul_mat_q_glu(*cuda_ctx, src0, src1, ids, glu, &fusion_data);
                    fused_mul_mat_vec = true;
                    fused_node_count  = 11;
                    break;
                }
            ...
            if (ggml_cuda_should_fuse_mul_mat_vec_q(up)) {
                ...
                if (!up->src[3] && !gate->src[3] && ggml_cuda_should_fuse_mul_mat_q_glu(up, fusion_data)) {
                    ggml_cuda_mul_mat_q_glu(*cuda_ctx, src0, src1, ids, glu, &fusion_data);
                    fused_mul_mat_vec = true;
                    fused_node_count  = 3;
                    break;
    ...
    if (node->op == GGML_OP_MUL_MAT && ggml_cuda_can_fuse_scaled_mul_mat(cgraph, i)) {
        ...
        if (ggml_cuda_extract_scaled_mul_mat(scaled, info) &&
            ...
            ggml_cuda_mul_mat_q(*cuda_ctx, info.mm->src[0], info.mm->src[1], nullptr, scaled, info.scale,
                    ggml_cuda_nvfp4_input_scale(info.mm));
            return 1;
        }
    }

...
| Experimental split-buffer repack pass, cache disabled | `6477.44 +/- 3.29 t/s` | `126.98 +/- 0.84 t/s` |
| Experimental split-buffer repack, `GGML_CUDA_NVFP4_REPACK_CACHE_MB=64` | `6488.39 +/- 9.84 t/s` | `126.95 +/- 0.92 t/s` |
| Experimental split-buffer repack, `GGML_CUDA_NVFP4_REPACK_CACHE_MB=128` | `6495.67 +/- 6.33 t/s` | `126.99 +/- 0.89 t/s` |
| Experimental split-buffer repack, `GGML_CUDA_NVFP4_REPACK_CACHE_MB=256` | `6480.65 +/- 12.12 t/s` | `126.77 +/- 0.82 t/s` |
| Experimental split-buffer repack, `GGML_CUDA_NVFP4_REPACK_CACHE_MB=512` | `6488.43 +/- 10.35 t/s` | `126.82 +/- 0.78 t/s` |
...
| Dropped coalesced-load NVFP4 activation quantizer | `6462.91 +/- 14.52 t/s` | `126.80 +/- 0.84 t/s` |
...
An NVFP4-only Blackwell X-tile stride experiment removed the four trailing padding int32 words from `MMQ_MMA_TILE_X_K_FP4`, reducing trace-reported dynamic shared memory from `28,928` to `27,904` bytes/block. Correctness passed, but it still did not cross the next CTA-per-SM threshold and the requested benchmark regressed to `6455.44 +/- 10.44 t/s` pp15000, `126.91 +/- 0.89 t/s` tg128. The change was dropped; keep the current 76-word stride because it preserves the existing `% 8 == 4` layout convention and benchmarks better on the 5070 Ti.
...
A coalesced-load activation quantizer prototype kept one thread per 16-value NVFP4 subblock but staged each 2048-value CTA tile through shared memory with coalesced global loads. Correctness passed for focused dense native NVFP4, MoE `MUL_MAT_ID type_a=nvfp4`, and dense repack-cache `MUL_MAT type_a=nvfp4`, but the requested benchmark regressed to `6462.91 +/- 14.52 t/s` pp15000 and `126.80 +/- 0.84 t/s` tg128. The code was dropped; the extra shared stores, shared loads, and barrier outweigh the global-load coalescing benefit.
